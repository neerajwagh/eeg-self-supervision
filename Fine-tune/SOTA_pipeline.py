...
if __name__ == '__main__':
	...
	# Dataset 
	train_dataset = TimeseriesDataset(window_idx=train_window_idx, X=X, y=y)
	valid_dataset = TimeseriesDataset(window_idx=val_window_idx, X=X, y=y)
	...
	for epoch in range(_EPOCH):
		...
		if epoch % 10 == 0:
			torch.save(state, f"{dataset}_{_TASK}_SOTA/_Epoch_{epoch}.ckpt")
