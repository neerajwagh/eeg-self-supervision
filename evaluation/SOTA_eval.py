...
from SOTA_model import RelativePositioningShallowNet
...
# This function is used to perform leave-1-out sampling and collect final metrics.
...
if __name__ == '__main__':
	...
	model.fc = torch.nn.Linear(100, 2)
	model.fc.weight.data.normal_(mean=0.0, std=0.01)
	model.fc.bias.data.zero_()

	# load trained model states
	file = f"../resources/finetuned_models/{dataset}_{_TASK}_sota.ckpt"
	...
	if _TASK == "condition" and dataset == "lemon":
		...
		for i in tqdm(range(len(heldout_indices_all))):
			heldout_dataset = TimeseriesDataset(window_idx=heldout_indices_all[i],X=X, y=y)
	...
	heldout_dataset = TimeseriesDataset(window_idx=heldout_window_idx, X=X, y=y)
	...
	print("Report-------------------------------")
	# print(f"heldout test AUROC: {np.mean(all_auc):.3f}({np.std(all_auc):.4f})")
