...
def _custom_cv_fold_iterator(train_and_val_subjects, num_folds, task, index_df, dataset, _RANDOM_SEED):

	print(len(train_and_val_subjects))
	if dataset == "tuh":
		keyword = "patient_id"
		if task == "gender":
			task_label = 'parsed_gender'
		elif task == "age":
			task_label = 'parsed_age_binary'
		elif task == "condition":
			task_label = "text_label"
	elif dataset == "lemon":
		keyword = "subject_id"
		task_label = task
	else:
	...
	y = [index_df.loc[index_df[keyword] == x][task_label].values[0] for x in train_and_val_subjects]
	...
	for (train_subject_idx, val_subject_idx), fold_idx in zip(cv_iter, range(num_folds)):
		...
		print(f"Fold {fold_idx}: train subjects: {len(train_subjects)} val subjects: {len(val_subjects)}")

		train_idx = index_df.index[index_df[keyword].isin(train_subjects)].tolist()
		val_idx = index_df.index[index_df[keyword].isin(val_subjects)].tolist()
